...
class Aster(BaseExchange):
    """Aster DEX - Binance-compatible API at fapi.asterdex.com"""
    name = "aster"
    native_interval_hours = 8

    async def fetch_symbols(self) -> list[tuple[str, str]]:
        data = await self._get("https://fapi.asterdex.com/fapi/v1/exchangeInfo")
        symbols = []
        ...
        # Aster uses Binance-compatible API, try fundingInfo
        await self._load_funding_intervals()
        return symbols

    async def _load_funding_intervals(self):
        try:
            data = await self._get("https://fapi.asterdex.com/fapi/v1/fundingInfo")
            ...
            pass  # Aster may not support this endpoint

    async def fetch_current_rates_batch(self) -> list[tuple[str, float]]:
        if not self._funding_intervals:
            await self._load_funding_intervals()
        data = await self._get("https://fapi.asterdex.com/fapi/v1/premiumIndex")
        rates = []
        for item in data:
            raw = item.get("symbol", "")
